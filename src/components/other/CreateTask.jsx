...
if(assignemployee && task && Submissiondate && Category && description){
  ...
  <div className="mt-5">
    <form action="">
      <div className=" ml-5  flex bg-stone-950 justify-between mr-5">
        <div className=" w-6/12 h-auto bg-black-700 p-2 m-5">
          ...
          <select required onChange={namehandler} className=" text-white w-full p-2 rounded bg-stone-800">
            ...
            {
            data.employee.map((name ,id=1)=>(<option key={id} className=" text-white">{name.firstname+" "+name.lastname}
            </option>))
            }
          </select>
        ...
        <div className="p-5">
          ...
          <button onClick={Createtaskhandler} className="w-96 text-gray-50 bg-green-500 font-bold p-2 rounded">Create Task</button>
        </div>
      </div>
     ...
     
      {
            ...
            ( 
              <div className=" bg-zinc-400 rounded flex justify-between m-5 ">
